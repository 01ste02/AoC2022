...
while IFS= read -r string; do
	...
	for i in $(seq 1 $num); do
	    ...
	    #if [[ "${stack_columns[$tcol]}" == *[_]* ]]; then
	#	tmpRes="$(sed "s/\(.*\)\[\_\]\(.*\)/\1$crate\2/g" <<< ${stack_columns[$tcol]})"
	 #       echo "	Result to: $tmpRes"
	  #      stack_columns[$tcol]="$tmpRes"
	   # else
            #    echo "	Result to: $crate ${stack_columns[$tcol]}"
	  #      stack_columns[$tcol]="$crate ${stack_columns[$tcol]}"
	   # fi
	 #   tmpRes="$(sed "s/\(.*\)$crate\(.*\)/\1\_\2/g" <<< ${stack_columns[$fcol]})"
	  #  echo "	Result from: $tmpRes"
	  #  stack_columns[$fcol]="$tmpRes"
	    echo "MOVE DONE"
	done
    elif [ ! -z "$string" ]; then
